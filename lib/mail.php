...
class Mail {
	...
	var $Encoding = "8bit";
	var $From = "customercare@bidcontender.com";
	var $FromName = "Bidcontender";
	var $to = array();
	var $cc = array();
	var $bcc = array();
	var $ReplyTo = array();
	...
	function create_header() {
		...
		if (count($this->attachment) > 0) {
			$header[] = sprintf("Content-Type: Multipart/Mixed; charset = \"%s\";", $this->CharSet);
			$header[] = sprintf(" boundary=\"Boundary-=%s\"\n", $this->boundary);
		} else {
			$header[] = sprintf("Content-Type: %s; charset = \"%s\";\n", $this->ContentType, $this->CharSet);
		}
		$header[] = "MIME-Version: 1.0\n\n";
		/* $fp = fopen('dmail.log.txt', 'a+');
		  fwrite($fp, "\r\n\r\n\r\n --------------------------------  ".date("Y-m-d H:i:s")."  --------------------------------\r\n");
	...
	function attach_all() {
		...
		$mime[] = "Content-Type: multipart/alternative; boundary=\"AltBoundary" . $this->boundary . "\"\n\n";
		$mime[] = "--AltBoundary" . $this->boundary . "\n";
...
//			printf("<font color=\"FF0000\">%s</font>", $msg);
	...
	function setMailFrom($femail, $fname) {
		$this->From = $femail;
		$this->FromName = $fname;
	}
	...
	function setMailContent($message) {
		$this->Body = $message;
	}
	...
	function attach($upFile) {
		$this->AddAttachment($upFile);
	}
